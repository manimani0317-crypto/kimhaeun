/*
  ...
  쓰는 법
    import { createRedButton, createCreamButton, createButtonRig } from './buttonModels.js';
...
/* ── 크림 단추 — 넓게 비탈진 테두리 + 오목한 가운데 + 점 구멍 링 + 십자 바느질 ── */
export function createCreamButton(THREE) {
  const mat = new THREE.MeshPhysicalMaterial({
    color: BUTTON_COLORS.cream, roughness: .55, metalness: 0,
    clearcoat: .25, clearcoatRoughness: .4, side: THREE.DoubleSide
  });

  const pts = [new THREE.Vector2(0, FLAT_Y)];
  pts.push(...smoothProfile(THREE, RIM_KEYS, 64));
  pts.push(new THREE.Vector2(0, 0));
  const body = new THREE.Mesh(new THREE.LatheGeometry(pts, 96), mat);

  /* 점 구멍 링 — 인스턴스 한 번으로 24개 */
  const DOTS = 24;
  const dots = new THREE.InstancedMesh(
    new THREE.CylinderGeometry(.022, .022, .012, 10),
    new THREE.MeshStandardMaterial({ color: 0x6d6255, roughness: .9 }),
    DOTS
  );
  const m = new THREE.Matrix4();
  for (let i = 0; i < DOTS; i++) {
    const a = i / DOTS * Math.PI * 2;
    m.makeTranslation(Math.cos(a) * .33, .072, Math.sin(a) * .33);
    dots.setMatrixAt(i, m);
  }

  /* 십자 바느질 — 실 두 가닥 */
  const threadMat = new THREE.MeshStandardMaterial({ color: BUTTON_COLORS.thread, roughness: .9 });
  const cross = new THREE.Group();
  [Math.PI / 4, -Math.PI / 4].forEach((ry, i) => {
    const t = new THREE.Mesh(new THREE.CapsuleGeometry(.034, .3, 4, 10), threadMat);
    t.rotation.z = Math.PI / 2;                        // 눕히고
    const holder = new THREE.Group();
    holder.rotation.y = ry;                            // X 모양으로 돌린다
    holder.position.y = .095 + i * .012;               // 위 가닥이 살짝 겹쳐 올라간다
    holder.add(t);
    cross.add(holder);
  });

  const g = new THREE.Group();
  g.add(body, dots, cross);
  return g;
}
